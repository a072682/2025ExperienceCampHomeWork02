...
const renderPageNumbers = () => {
    ...
    function renderPageItems(pages, currentPage, handlePageChange) {
        return pages.map((page, index) => {//對pages矩陣進行.map展開 例如:[1, 2, 3, 4, '...']
            ...
            return (
                ...
                <span className="page-link border-0 w40 h40">...</span>
                </li>
            );
            } else {
            return (
                ...
                <button className="page-link border-0 w40 h40" onClick={() => handlePageChange(page)}>
  ...
  return (
    <div className="container text-center">
      ...
      <ul className="pagination w328 py-8 px-20 justify-content-center align-items-center mx-auto mb-0">
        <li className={`page-item ${currentPage === 1 ? 'disabled' : ''}`}>
          <button className="page-link w40 h40 d-flex align-items-center border-0" onClick={() => handlePageChange(currentPage - 1)}>
            <span className="material-symbols-outlined fs-24">arrow_back_ios</span>
          </button>
        </li>
       ...
       pages進入到renderPageItems進行輸出頁碼 */}
        ...
        <li className={`page-item ${currentPage === totalPages ? 'disabled' : ''}`}>
          <button className="page-link w40 h40 d-flex align-items-center border-0" onClick={() => handlePageChange(currentPage + 1)}>
            <span className="material-symbols-outlined fs-24">arrow_forward_ios</span>
          </button>
        </li>
      </ul>
    </div>
